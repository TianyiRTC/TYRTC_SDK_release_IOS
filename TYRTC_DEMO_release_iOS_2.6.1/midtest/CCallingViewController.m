...
SDK_GROUP_TYPE grpType=SDK_GROUP_CHAT_AUDIO;//会议类型
int isGroup;//0表示点对点,1表示主动发起多人,2表示主动参加多人，多人来电时置为1或2均可
#endif
@interface CCallingViewController ()
{
    int mRotate;//旋转摄像头角度SDK_VIDEO_ROTATE
    ...
    //NSTimer* mCallDurationTimer;//通话时长计时器
}
@property (strong, nonatomic) UIView *localVideoView;//本地窗口
@property (strong, nonatomic) DAPIPView *dapiview;//画中画
@property (strong, nonatomic) IOSDisplay *remoteVideoView;//远端窗口
...
- (void)viewDidLoad
{
    ...
    [vItem release];
    //callingview下挂dapiview和remoteVideoView，dapiview下挂localVideoView
    //dapiview(DAPIPView类)控制本地视频采集，并显示到localVideoView
    //remoteVideoView(IOSDisplay类)控制远端视频采集和显示
    ...
    //只要callingview被创建并present，就会进入viewdidload
    //从而触发onSendVideoParam，onSendVideoParam的作用是抛出呼叫事件，给ViewController监听
...
/**************************************控件响应*****************************************/
//ccallingview的作用是抛出事件，给ViewController执行

//发起呼叫
-(void)onSendVideoParam
{
    long long rVideo = 0;//定义为long long类型以适配arm64
    long long lVideo = 0;
...
#endif
                            [NSNumber numberWithInt:0],@"arg",
                            [NSNumber numberWithLongLong:rVideo],@"rvideo",//将窗口指针传递给ViewController
...
//打开以下代码可以录制视频，保存录像是有一定时间等待的，请将抛出录像事件放在异步线程，以防死锁
...
//通话已接通，在这里控制button的隐藏和显示，若button显示异常请检查这里的判定条件
